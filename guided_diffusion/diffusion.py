...
# 文件: guided_diffusion/diffusion.py (文件顶部)
from skimage.metrics import structural_similarity as ssim
import lpips
# structure_utils.py
import torch
import kornia # kornia是一个非常方便的、基于PyTorch的图像处理库
...
class Diffusion(object):
    ...
    def svd_based_ddnm_plus(self, model, cls_fn): 
        ...
        log_file_path = os.path.join(self.args.image_folder, "metrics.log")

        # Configure logging to write to the specified file
        # We remove any existing handlers to avoid duplicate logs if the function is called multiple times
        for handler in logging.root.handlers[:]:
            logging.root.removeHandler(handler)
        
        logging.basicConfig(
            level=logging.INFO,
            format='INFO - %(module)s.py - %(asctime)s - %(message)s',
            handlers=[
        ...
        idx_init = args.subset_start
        idx_so_far = args.subset_start
        loss_fn_lpips = lpips.LPIPS(net='alex').to(self.device)
        avg_ssim = 0.0
        avg_lpips = 0.0
        avg_psnr = 0.0
        pbar = tqdm.tqdm(val_loader)
        ...
        for i,(x_orig, classes) in enumerate(pbar):
            ...
            for i in range(len(Apy)):
                ...
                tvu.save_image(
                    inverse_data_transform(config, x_orig[i]),
                    os.path.join(self.args.image_folder, f"orig/{idx_so_far + i}.png")
                )
            ...
            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, f"results/{idx_so_far + j}_{0}.png")
                )
                orig = inverse_data_transform(config, x_orig[j])
                restored = x[0][j] # x[0][j] 是已经逆转换为 [0, 1] 范围的图像
                mse = torch.mean((x[0][j].to(self.device) - orig) ** 2)
                psnr = 10 * torch.log10(1 / mse)
                avg_psnr += psnr
                # 计算 SSIM
                # scikit-image需要numpy数组，且通道在最后
                orig_np = orig.cpu().numpy().transpose(1, 2, 0)
                restored_np = restored.cpu().numpy().transpose(1, 2, 0)
                # data_range是像素值的范围，对于[0,1]的图像，它是1.0
                current_ssim = ssim(orig_np, restored_np, data_range=1.0, channel_axis=2, multichannel=True)
                avg_ssim += current_ssim

                # 计算 LPIPS
                # lpips库需要[-1, 1]范围的Tensor，所以我们要重新转换一下
                orig_lpips = orig * 2 - 1
                restored_lpips = restored * 2 - 1
                current_lpips = loss_fn_lpips(orig_lpips.to(self.device), restored_lpips.to(self.device))
                avg_lpips += current_lpips.item()
                # =================================================================================
                img_ind = idx_so_far + j
                total_processed = img_ind - idx_init + 1
                # 打印当前单张图片的指标
                print(f"img_ind: {img_ind}, PSNR: {psnr.item():.2f}, SSIM: {current_ssim:.3f}, LPIPS: {current_lpips.item():.4f}")

                # 计算并打印当前的平均指标
                running_avg_psnr = avg_psnr.item() / total_processed
                running_avg_ssim = avg_ssim / total_processed
                running_avg_lpips = avg_lpips / total_processed
            
                print(f"Avg PSNR: {running_avg_psnr:.2f}, Avg SSIM: {running_avg_ssim:.4f}, Avg LPIPS: {running_avg_lpips:.4f}   (** After {total_processed} images **)")

                # Log metrics for the current individual image
                logging.info(f"img_ind: {img_ind}, PSNR: {psnr.item():.2f}, SSIM: {current_ssim:.3f}, LPIPS: {current_lpips.item():.4f}")

                # Calculate and log the running average metrics
                running_avg_psnr = avg_psnr.item() / total_processed
                running_avg_ssim = avg_ssim / total_processed
                running_avg_lpips = avg_lpips / total_processed
            
                logging.info(f"Avg PSNR: {running_avg_psnr:.2f},Avg SSIM: {running_avg_ssim:.4f}, Avg LPIPS: {running_avg_lpips:.4f}   (** After {total_processed} iteration **)")
                # =================================================================================


            idx_so_far += y.shape[0]
            images_processed = idx_so_far - idx_init
            elapsed_time = time.time() - start_time
            avg_time_per_image = elapsed_time / images_processed

            pbar.set_postfix_str("Time: %.2f s/img" % avg_time_per_image)
            #pbar.set_description("PSNR: %.2f, SSIM: %.3f, LPIPS: %.3f" % 
        ...
        avg_ssim /= (idx_so_far - idx_init)
        avg_lpips /= (idx_so_far - idx_init)
        print(f"Total Average PSNR: {avg_psnr:.2f}, SSIM: {avg_ssim:.3f}, LPIPS: {avg_lpips:.3f}")
        print("Number of samples: %d" % (idx_so_far - idx_init))
# Code form RePaint   
